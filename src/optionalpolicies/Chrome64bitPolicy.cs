...
namespace windows_security_tweak_tool.src.optionalpolicies
{
    class Chrome64bitPolicy : OptionalPolicy
    {
        ...
        public bool ApplyAsync()
        {
                ...
                if (!Directory.Exists(Config.GetConfig().GetDataFolder() + @"\wstt-downloaded\chrome"))
                {
                    Directory.CreateDirectory(Config.GetConfig().GetDataFolder() + @"\wstt-downloaded\chrome");
                }

            WebClient client = new WebClient();
            client.DownloadFile("https://dl.google.com/tag/s/appguid%3D%7B8A69D345-D564-463C-AFF1-A69D9E530F96%7D%26iid%3D%7B03FE9563-80F9-119F-DA3D-72FBBB94BC26%7D%26lang%3Den%26browser%3D4%26usagestats%3D0%26appname%3DGoogle%2520Chrome%26needsadmin%3Dprefers%26ap%3Dx64-stable%26brand=GCEB/dl/chrome/install/GoogleChromeEnterpriseBundle64.zip", Config.GetConfig().GetDataFolder() + @"\wstt-downloaded\chrome\chrome.zip");
            FileStream fs = File.OpenRead(Config.GetConfig().GetDataFolder() + @"\wstt-downloaded\chrome\chrome.zip");
